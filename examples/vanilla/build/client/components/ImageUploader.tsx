...
export default function ImageUploader({ processImage, updateImage, onImageUpdated }: ImageUploaderProps) {
    ...
    return (<div className="loading-box pt1" data-loading={isPending}>
      <form action={formAction}>
        <label htmlFor="image" className="p1">
          <span className="btn" aria-disabled={isPending}>
            {isPending ? "Reading the image ..." : "Upload image"}
          </span>
          <input type="file" id="image" name="image" accept="image/*" required className="hide" onChange={uploadImage} disabled={isPending}/>
        </label>
      </form>
      {!isPending && processedImage && (<Image className="mt1" id={processedImage.id}>
          <ul className="reset">
            {processedImage.suggestions.map((item, index)=>(<li key={index}>
                <button className="reset" onClick={()=>setImageContent(processedImage.id, item.label)} disabled={isImageUpdating}>
