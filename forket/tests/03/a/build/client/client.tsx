...
import f_6 from "./components/Footer.tsx";
import f_5 from "./components/Expandable.tsx";
window.$f_5 = f_5;
window.$f_6 = f_6;
...
(() => {
  ...
  (function() {
    ...
    function FRSC_init() {
      ...
      $F_logc(`\u{10090} [client] \u{1F3DD}\uFE0F(${islands.length}) v${"0.7.9"}`);
      ...
      function hydrateComponentBetweenMarkers(id, componentName, props, children) {
        renderers[id] = function(newProps) {
          ...
          if (!Component) {
            console.warn(`\u{10090} Component <${componentName}> not found in the global scope yet. (${id})`);
            return false;
          }
          const boundary = {
          ...
          renderers[id] = function(newProps2) {
            ...
            $F_logc(`\u{10090} [client] Render <${componentName}> (${id})`, { props, children });
    ...
    function FLP_process(id) {
      ...
      if (typeof window.$FLP_ !== "undefined" && window.$FLP_[id]) {
        ...
        if (status === "resolved") {
          $F_logc(`\u{10090} [client] Promise resolved (${id})`, value);
          if (resolve) {
            resolve(value);
            delete window.$FLP_[id];
          } else {
            $F_logc(
              `\u{10090} [client] Promise with id ${id} is resolved but can't be consumed because the client boundary that needs it is not render yet.`
            );
          }
        } else if (status === "rejected") {
          $F_logc(`\u{10090} [client] Promise rejected (${id})`, value);
          if (reject) {
            delete window.$FLP_[id];
            reject(new Error(value));
          } else {
            $F_logc(
              `\u{10090} [client] Promise with id ${id} is rejected but can't be consumed because the client boundary that needs it is not render yet.`
            );
          }
        } else {
    ...
    function FSA_call(id, funcName) {
      return async function(...args) {
        ...
        if (typeof FormData !== "undefined" && data instanceof FormData) {
          ...
          const result2 = await fetch($F_sae + "/" + funcName, {
            method: "POST",
            body: fd
          });
          if (!result2.ok) {
